...
async def weakness_chart(bot: discord.Client, message: discord.Message):
    ...
    display_type = fetch_user_utility_setting_cache_by_user_name(
        user_name, "battle_weakness"
    )
    ...
    if display_type is None and user_name not in not_weakness_chart_user_names:
        ...
        if member:
            different_name = True
            display_type = fetch_user_utility_setting_cache(member.id, "battle_weakness")
