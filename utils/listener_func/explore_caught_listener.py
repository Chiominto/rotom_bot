...
from utils.cache.cache_list import processed_explore_messages, celestial_members_cache
...
async def explore_caught_listener(
    ...
    # Get member info
    member: discord.Member = await get_pokemeow_reply(before_message)
    if not member:
        return
    ...
    if clan_member_role not in member.roles:
        return
