...
def format_display_boss_name(boss_name: str) -> str:
    ...
    if "Shiny Gigantamax" in boss_name:
        boss_name = boss_name.replace(
            "Shiny Gigantamax-", f"{Emojis.shinygigantamax} "
        )
    elif "Gigantamax" in boss_name:
        boss_name = boss_name.replace("Gigantamax-", f"{Emojis.gigantamax} ")
    elif "Shiny Eternamax" in boss_name:
        boss_name = boss_name.replace("Shiny Eternamax-", f"{Emojis.shinygigantamax} ")
    elif "Eternamax" in boss_name:
        boss_name = boss_name.replace("Eternamax-", f"{Emojis.gigantamax} ")
...
async def register_wb_battle_reminder(
    ...
    if not user_alert_setting or user_alert_setting.lower() != "on":
        ...
        return
    guild = message.guild
    user_info = celestial_members_cache.get(member.id)
    channel_id = user_info.get("channel_id") if user_info else None
    public_channel = guild.get_channel(CELESTIAL_TEXT_CHANNELS.shuckles_swamp)
    notify_channel = None
    if not channel_id:
